...
export function categorizeEmail(email, deals) {
  ...
  for (const deal of deals) {
    // Fall back to matching the deal name itself if no keywords are defined
    const keywords = deal.keywords?.length > 0 ? deal.keywords : [deal.name]
    const dealScore = keywords.filter(kw => text.includes(kw.toLowerCase())).length
